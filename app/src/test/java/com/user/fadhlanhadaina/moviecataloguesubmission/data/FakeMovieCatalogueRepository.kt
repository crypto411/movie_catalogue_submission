...
import com.user.fadhlanhadaina.moviecataloguesubmission.data.source.MovieCatalogueDataSource
...
class FakeMovieCatalogueRepository(private val remoteDataSource: RemoteDataSource, private val localDataSource: LocalDataSource):
        ...
        remoteDataSource.getTVSeriesDetail(id, object: RemoteDataSource.TVSeriesDetailCallback {
            override fun onTVSeriesReceived(tvSeries: TVSeries) {
                tvSeries.genres.forEachIndexed { index, genre ->
                    tvSeries.stringGenres = if(index == 0)
                        genre.name
